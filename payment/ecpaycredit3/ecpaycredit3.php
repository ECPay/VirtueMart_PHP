...
class plgVmPaymentECPayCredit3 extends vmPSPlugin {
        ...
        if ($order) {
            try {
                ...
                array_push($aio->Send['Items'], array(
                    'Name' => vmText::_('VMPAYMENT_ECPAY_CREDIT3_ORDER_INFO'),
                ...
                $aio->SendExtend['CreditInstallment'] = 3;
                ...
                $szHtml .= $this->getHtmlRow('ECPAY_CREDIT3_PAYMENT_METHOD', $this->renderPluginName($this->_currentMethod), "vmorder-done-payinfo");
                $szHtml .= $this->getHtmlRow('ECPAY_CREDIT3_ORDER_NUMBER', $order['details']['BT']->order_number, "vmorder-done-nr");
                $szHtml .= $this->getHtmlRow('ECPAY_CREDIT3_ORDER_TOTAL', $oCurrency->priceDisplay($order['details']['BT']->order_total), "vmorder-done-amount");
                $szHtml .= $this->getHtmlRow('ECPAY_CREDIT3_RESPONSE', vmText::_('VMPAYMENT_ECPAY_CREDIT3_RESPONSE_PROCESSING') . $aio->CheckOutString('結帳'));
                $szHtml .= '</table>' . "\n";
    ...
    function plgVmOnPaymentResponseReceived(&$html) {
        ...
        try {
            ...
            if (sizeof($arFeedback) > 0) {
                ...
                if ($deTradeAmount == $deTotalAmount) {
                    ...
                    if ($szReturnCode == 1 || $szReturnCode == 800) {
                        $szComment = vmText::_('VMPAYMENT_ECPAY_CREDIT3_RESPONSE_SUCCESS');
                        // 更新訂單資料與新增訂單歷程。
                        ...
                        $html .= $this->getHtmlRow('ECPAY_CREDIT3_PAYMENT_INFO', $this->renderPluginName($this->_currentMethod), "vmorder-done-payinfo");
                        $html .= $this->getHtmlRow('ECPAY_CREDIT3_ORDER_NUMBER', $szOrderID, "vmorder-done-nr");
                        $html .= $this->getHtmlRow('ECPAY_CREDIT3_ORDER_TOTAL', $oCurrency->priceDisplay($oOrder['details']['BT']->order_total), "vmorder-done-amount");
                        $html .= $this->getHtmlRow('ECPAY_CREDIT3_RESPONSE', $szComment . ' ' . vmText::_('VMPAYMENT_ECPAY_CREDIT3_RESPONSE_PENDING'));
                        $html .= '</table>' . "\n";
                    } else {
            ...
            $html = '<table class="vmorder-done">' . "\n";
            $html .= $this->getHtmlRow('ECPAY_CREDIT3_PAYMENT_INFO', $this->renderPluginName($this->_currentMethod), "vmorder-done-payinfo");
            $html .= $this->getHtmlRow('ECPAY_CREDIT3_ORDER_NUMBER', $szOrderNumber, "vmorder-done-nr");
            $html .= $this->getHtmlRow('ECPAY_CREDIT3_RESPONSE', vmText::_('VMPAYMENT_ECPAY_CREDIT3_RESPONSE_FAIL'));
            $html .= '</table>' . "\n";
            // 顯示錯誤
            vmError($html);
        }
